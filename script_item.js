...
const backendIPAddress = 'http://54.221.188.147:3000';
...
const getItemsFromDB = async () => {
  ...
  myHeaders.append(
    'Cookie',
    'connect.sid=s%3AWo7NyvK85A-DyGLW9GgedlNPNpVdeQeO.NFieR5likbFQssSo4ioe%2FuWgnXmIVa2PzRAwzEOylIk'
  );
  ...
  itemsData = await fetch(`http://${backendIPAddress}/items`, requestOptions)
...
const addItem = async () => {
  ...
  myHeaders.append(
    'Cookie',
    'connect.sid=s%3A-mVRjXMtN3AJuD1nE1JkKd97KiqO_DMd.AGtJB6SDtk07rByt9n%2Fp0z3wtTrf%2FZ5O%2BD7iGVD%2FKEA'
  );
  ...
  fetch(`http://${backendIPAddress}/items`, requestOptions)
    .then(() => redrawDOM())
...
const deleteItem = async (item_id) => {
  ...
  fetch(`http://${backendIPAddress}/items/${item_id}`, requestOptions)
    .then(() => redrawDOM())
    .catch((error) => console.log('error', error));
};
